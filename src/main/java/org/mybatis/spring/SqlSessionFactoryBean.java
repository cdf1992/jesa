...
import jes.pub.runtime.JesDebugException;
import jes.utils.JdbcUtil;
...
public class SqlSessionFactoryBean
  ...
  public void setMapperLocations(Resource... mapperLocations) {
    ...
    // TODO 迁移 springboot，为支持基于文件夹的方言体系重写此方法
    if (this.dataSource == null) {
      throw new JesDebugException("使用 MapperLocations 时请注入 dataSource");
    }
    String dialect = "/" + JdbcUtil.getDatabaseInfo(this.dataSource).getDbType().toString() + "/";
    ArrayList<Resource> dialectResource = new ArrayList<Resource>();
    for (Resource resource : mapperLocations) {
      try {
        if (resource.getURL().getPath().contains(dialect)) {
          dialectResource.add(resource);
        }
      } catch (IOException e) {
        throw new JesDebugException("使用 MapperLocations 时指定的资源路径或类型错误 [" + resource + "]");
      }
    }
    this.mapperLocations = dialectResource.toArray(new Resource[dialectResource.size()]);
  }

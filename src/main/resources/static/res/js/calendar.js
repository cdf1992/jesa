;(function (global, undefined) {
    ...
    // Default style
    var defaultStyle = '#calendarWrap {width: 100%;text-align: center;border: 1px solid #ebe8e8;}\n' +
        ...
        '        #calendarTable {border-collapse: collapse;border-bottom: 1px solid #ebe8e8;}\n' +
    ...
    Calendar.prototype.getCalendar = function (year, month) {
        ...
        for (var i = 0; i < 7; i++) {
            if (i <= lastday) {
                ...
                var lastStr = lastYear + '-' + lastMonth + '-' + cache
                ...
                if (thisDateYear == year && thisDateMonth == month && today == t) {
                    str += '<td class="hightlightToday calendar-cell" data-date="' + thisDateYear + '-' + thisDateMonth + '-' +
                        today + '"><div>' + t++ + '</div></td>'
                } else {
                    var cache2 = t++
                    var thisStr = year + '-' + month + '-' + cache2
    ...
    Calendar.prototype.addData = function () {
        var data = this.defaultOption.data

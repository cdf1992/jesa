...
Block.prototype.getHtml = function()
{
    ...
    if(this.blocks.getOption('editable', true)){
        html += '<div class="blockicon delete"></div>'
    }
    ...
    if(this.blocks.getOption('editable', true)){
         html += '<div class="blockicon settings"></div>';
    }
    html += '<div class="infos"></div></div>';
...
{
    var html = '<div id="block' + this.id + '" class="block ' + this.meta['cls'] + '"></div>'
...
Block.prototype.initListeners = function()
{
    ...
    if(self.blocks.getOption('editable', true)){
         // Draw a link
...
Blocks.prototype.run = function(selector)
{
    ...
    $(document).ready(function() {
        ...
        self.div.html(
            ...
            + '<div class="blocks text-ellipsis"></div>'
            + '</div>'
        );
        ...
        if(!self.getOption('disableMenu', false)){
            // Run the menu
        ...
        if(self.getOption('editable', true)){
...
{
    if (this.getOption('editable', true) && this.selectedSide) {
    ...
    if (!this.selectedBlock) {
        for (var k in this.edges) {
            ...
            if (collide != false) {
                ...
                var self = this;
                var selectedNext = function(id){
                    var eds = self.edgeMap[id];
                    if(eds){
                        for(var n in eds){
                            eds[n].selected = true;
                            selectedNext(eds[n].block2.id);
                        }
                    }
                }
                selectedNext(this.edges[k].block2.id);
...
Blocks.prototype.doRedraw = function()
{
    ...
    if (this.getOption('editable', true) && this.linking) {
        try {
    ...
    this.ready(function() {
            ...
            for (var k in scene.edges) {
                try {
                    ...
                    if(!self.edgeMap[edge.block1.id]){
                        self.edgeMap[edge.block1.id] = [];
                    }
                    self.edgeMap[edge.block1.id].push(edge);
            ...
            if (errors.length) {
                ...
                text += '</ul>';
                self.messages.show(text, {'cls': 'error'});
            }

...
Ext.define('Ext.ux.TreePicker', {
    ...
    createPicker: function() {
        var me = this;
        var listeners = {scope: me};
        var config = {};
        if(me.multiSelected){
            config['bbar'] = [
                { xtype: 'button', text: '确定', handler : me.onButtonClick, scope : me }
            ]
        } else {
            listeners['itemclick'] = me.onItemClick;
        }
        var picker = new Ext.tree.Panel(Ext.applyIf({
                shrinkWrapDock: 2,
                store: me.store,
                floating: true,
    ...
    , setValues: function(values) {
        ...
        if (null == values || me.store.loading) {
            // Called while the Store is loading. Ensure it is processed by the onLoad method.
            return me;
        }
        ...
        for(var i = 0; i < values.length; i++){
            var v = values[i];
            var record = me.store.getNodeById(v);
            if(record){
                value += record.getId() + ',';
        ...
        me.value = value.substr(0, value.length - 1);
        me.setRawValue(display.substr(0, display.length - 1));

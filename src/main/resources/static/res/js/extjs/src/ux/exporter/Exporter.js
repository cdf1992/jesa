...
Ext.define("Ext.ux.exporter.Exporter", {
    ...
    statics: {
        ...
        exportGrid: function(grid, formatter, config) {
            ...
            var columns = [],
                gridcm = grid.headerCt ? grid.headerCt.items.getRange() : grid.columns;
            Ext.each(gridcm, function(col){
                if(!col.hidden && col.xtype != "actioncolumn" && col.xtype != "rownumberer"){
                    var cfg = {};
                    if(Ext.isFunction(col.renderer)){
                        cfg['renderer'] = col.renderer;
                    }
                    columns.push(Ext.apply(col.initialConfig, cfg));
                }
            });

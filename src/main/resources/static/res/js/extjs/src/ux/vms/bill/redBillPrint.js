 ...
 */
Ext.define("Ext.ux.vms.bill.redBillPrint", {
			alias : 'Ext.ux.vms.bill.redBillPrint'
		})
 ...
 */
var redBillPrint = function(kpStatus, grid, gridStore, vmskpRelyon,invoiceTaxControl) {
	if (grid.length > 0) {
		var ids = [];
		for(var i=0;i<grid.length;i++){
		 ...
		 Ext.MessageBox.alert('提示','请选择需要打印的数据');
	 }
	var reqUrl = "";
	if (vmskpRelyon == "2") {
	...
	switch (kpStatus) {
		...
		case 2 :
			break;
		case 3 :
			reqUrl = 'createRedBillPrint.ajax';
			break;
		case 4 :
